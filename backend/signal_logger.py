                ...
                if _current_position and ist.hour >= 15 and ist.minute >= 29:
                    try:
                        ...
                        if not sig.get('error'):
                            ...
                            # ── Log BTST Signal (if present) ──────────────
                            btst_action = sig.get('btst_action', 'NO BTST')
                            if btst_action != 'NO BTST':
                                btst_reasons = " | ".join(sig.get('btst_reasons', [])[:4])
                                _post_to_sheet({
                                    "type": "BTST_SIGNAL",
                                    "entry_time": ist.strftime("%H:%M:%S"),
                                    "spot_price": sig.get('current_price', 0),
                                    "trade": f"{sig.get('atm_strike', '')} {btst_action}",
                                    "entry_premium": sig.get('atm_ce_ltp', 0) if 'CE' in btst_action else sig.get('atm_pe_ltp', 0),
                                    "exit_time": "NEXT DAY",
                                    "exit_spot": "",
                                    "exit_premium": "",
                                    "pnl_points": "",
                                    "pnl_rupees": "",
                                    "confidence": sig.get('btst_confidence', 0),
                                    "signal_strength": "",
                                    "mtf": sig.get('mtf_confluence', ''),
                                    "reasons": btst_reasons,
                                    "remark": f"BTST Entry:{sig.get('btst_entry','')} SL:{sig.get('btst_sl','')} TGT:{sig.get('btst_target','')}",
                                    "duration_mins": "OVERNIGHT",
                                })
                                _signal_log.append({
                                    "type": "BTST_SIGNAL",
                                    "trade": btst_action,
                                ...
                                print(f"[SignalLogger] 🌙 BTST logged: {btst_action} (Conf: {sig.get('btst_confidence', 0)}%)")
                            else:
                                _post_to_sheet({
                                    "type": "BTST_SIGNAL",
                                    "entry_time": ist.strftime("%H:%M:%S"),
                                    "spot_price": sig.get('current_price', 0),
                                    "trade": "NO BTST — Skip Tonight",
                                    "entry_premium": "",
                                    "exit_time": "",
                                    "exit_spot": "",
                                    "exit_premium": "",
                                    ...
                                    "signal_strength": "",
                                    "mtf": "",
                                    "reasons": " | ".join(sig.get('btst_reasons', [])[:3]),
                                    "duration_mins": "",
                                })
                                print(f"[SignalLogger] 🌙 BTST: No setup tonight (Conf: {sig.get('btst_confidence', 0)}%)")
